package com.ry.gitproject.util;
...
public class BarTextColorUtils {
    ...
    public static int StatusBarLightMode(Activity activity){
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if(MIUISetStatusBarLightMode(activity, true)){//判断是不是小米系统
                result=1;
            }else if(FlymeSetStatusBarLightMode(activity.getWindow(), true,activity)){//判断是不是魅族系统
                result=2;
     ...
     * @return
     */
    public static boolean FlymeSetStatusBarLightMode(Window window, boolean dark,Activity activity) {
        boolean result = false;
        if (window != null) {
            try {
                ...
                if (dark) {
                    StatusbarColorUtils.setStatusBarDarkIcon(activity, Color.BLACK);
                } else {
                    value &= ~bit;
                }
